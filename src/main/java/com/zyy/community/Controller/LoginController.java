...
//        String kaptcha = (String) session.getAttribute("kaptcha");
    ...
    public String register(Model model, User user){
        ...
        if(map == null || map.isEmpty()){
            model.addAttribute("msg", "我们已经向您的邮箱发送了一封邮件，请前去激活");
    ...
    public String activation(Model model, @PathVariable("userId") int userId,
        ...
        int status = userService.activate(userId, code);
        if(status == ACTIVATION_SUCCESS ){
            model.addAttribute("msg", "激活成功，可以正常使用了！");
            model.addAttribute("target", "/login");
        }else if(status == ACTIVATION_REPEAT){
            model.addAttribute("msg", "无效请求，返回首页");
            model.addAttribute("target", "/index");
        }else{
            model.addAttribute("msg", "激活失败，返回首页");
            model.addAttribute("target", "/index");
        }
        return "/site/operate-result";
    }
